...
class Simulation:
    def __init__(self, CanvasCarte):
        ...
        #couleurs possibles pour les equipes :
    ...
    def generationCarte(self, nbCells, densite, tailleStationRecharge, tailleLieuxMission):
        ...
        self.genererEquipes()
        print('Equipe 1 :', self.equipes[0].name, self.equipes[0].robots)
        print('Equipe 2 :', self.equipes[1].name, self.equipes[1].robots)
    ...
    def genererEquipes(self, nbEquipes = 2) :
        nbRobots=int(self.carte.nx / (nbEquipes*4))
        ...
        for i in range (nbEquipes) :
            ...
            self.equipes.append(Equipe(name, color))
            for j in range(nbRobots) :
                robot = self.ajouterRobot('test', self.equipes[i])
    ...
    def ajouterRobot(self, name, equipe : Equipe) -> Robot :
        robot = equipe.ajouterRobot(name, self.carte.getCelluleRandom(), self.carte, self)
